...
namespace Velvet.GameSystems
{
    public class CameraScroller : CameraEffect
    {
        ...
        private bool scrollInitialized { get; set; } = false;
        ...
        private float timeInMilliseconds { get; set; }
        ...
        //Side Vectors
        Vector2 RightSide => new Vector2(Camera.WorldBounds.Right, 0);
        Vector2 LeftSide => new Vector2(Camera.WorldBounds.Left, 0);
        Vector2 TopSide => new Vector2(0, Camera.WorldBounds.Top);
        Vector2 BottomSide => new Vector2(0, Camera.WorldBounds.Bottom);
        ...
        private void ScrollToTarget(Vector2 targetPosition, float timeInMilliseconds, float acceleration, double deltaTime)
        {
            ...
            nextPosition = new Vector2(Camera.Position.X + xAmt, Camera.Position.Y + yAmt);
        ...
        private void InfiniteScroll(Vector2 targetPosition, float timeInMilliseconds, float acceleration, double deltaTime)
        {
            ...
            if (!scrollInitialized)
            {
                ...
                var dep = new MethodPositionDependency(GetCurrentPosition);
        ...
        {
            return $"si: {scrollInitialized} | targ: {targetPosition} | s: {Scrolling} | xr {XTargetReached()} | yr {YTargetReached()}";
        }
        #endregion
